...
\begin{figure*}
 ...
 \caption{Platform overview. (a) Single-channel powder-doser module (CAD render; photograph of the printed platform will replace this render): printed Archimedes auger, printed spur-gear drive from a NEMA-11 stepper, tap collar carrying the solenoid striker and ERM vibration motor, and servo-tilted hinged mounting plate on the baseplate. (b) Schematic of the powder-flow path through the module: powder is loaded through slots into the auger tube, which is itself the reservoir (no separate hopper), and is conveyed down the helical channel $\rightarrow$ exit nozzle $\rightarrow$ collection cup on the analytical balance. (c) Tilt sweep about the fixed dispense point: the hinge axis passes through the exit-nozzle tip so the dispense point does not translate as the channel tilts from 0$^\circ$ (horizontal park) to 90$^\circ$ (vertical dispense). (d) Closed-loop gravimetric dosing concept: the target mass enters the controller, which drives the actuation while the balance feeds the measured mass back. (e) Compressed design timeline; the complete 97-entry design log is available in the repository.}
 \label{fgr:overview}
\end{figure*}
...
\centering
  ...
  \caption{Future work: multi-powder doser. (a) $N$-channel radial array concept around a shared collection cup on a single balance. (b) CAD study of inward-tilting channels over the shared cup.}
  \label{fgr:future}
\end{figure}
