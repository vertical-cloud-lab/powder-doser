...
Doses are dispensed onto a tared collection cup on the load cell (laboratory balance cross-check at 1~mg resolution), with $n\ge10$ replicates per condition. Per-powder calibration sweeps auger speed at fixed dose; closed-loop accuracy is evaluated across 20~mg--5~g requested masses with coarse-then-trickle control. The controller dispenses in coarse mode (continuous rotation at the calibrated mass-flow rate) until 90\% of the target mass is reached, then switches to trickle mode (single-step increments with tap/vibration bursts), sampling the HX711 at 10~Hz with a median-of-5 filter and accepting readings only in quiet windows $\ge$0.5~s after the last actuation; dosing terminates when the filtered mass reaches the target or a configurable overshoot/timeout bound trips. Controller pseudocode and parameters ship with the firmware. The protocol, scripts, and (forthcoming) raw data accompany the repository.
...
\centering
  ...
  \caption{Future work: multi-powder doser. (a) $N$-channel radial array concept around a shared collection cup on a single load cell. (b) CAD study of inward-tilting channels over the shared cup.}
  \label{fgr:future}
\end{figure}
