...
\begin{figure*}
 ...
 \caption{Platform overview. (a) Single-channel powder-doser module (CAD render; photograph of the printed platform will replace this render): printed Archimedes auger, gear drive from a NEMA-11 stepper, tap collar with solenoid and vibration motor, and servo-tilted hinged mounting plate on the baseplate. (b) Powder-flow path through the module cross-section (early v2-module render): loading slots $\rightarrow$ helical channel $\rightarrow$ exit nozzle $\rightarrow$ collection cup on the balance; in the final design the auger tube itself is the powder reservoir, with no separate hopper. (c) Tilt sweep about the fixed dispense point: the hinge axis passes through the exit-nozzle tip so the dispense point does not translate as the channel tilts from 0$^\circ$ (sealed park) to 90$^\circ$ (vertical dispense). (d) Closed-loop gravimetric dosing concept. (e) Compressed design timeline; the complete 97-entry design log is available in the repository.}
 \label{fgr:overview}
\end{figure*}
