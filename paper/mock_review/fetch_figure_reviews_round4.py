...
figure_review_round4.py.

Reads figure_review_round4_task_ids.json (flat {"tasks": {fig: id}}), polls each
task, and writes results/verify4__<figure>.answer.md (+ .notebook.ipynb) and a
results/INDEX_round4.md status roll-up.

Usage:
  export EDISON_API_KEY=...   # or EDISON_PLATFORM_API_KEY
  python fetch_figure_reviews_round4.py [--minutes 20]
"""
...
IDS = HERE / "figure_review_round4_task_ids.json"
...
def main() -> None:
    ...
    while True:
        ...
        for fig, tid in pending:
            try:
                ...
                print(f"  verify4__{fig} get_task error: {exc}", flush=True)
                continue
            status = str(data.get("status", "")).lower()
            if status in {"success", "fail", "cancelled", "truncated"}:
                answer = _answer(data)
                (RESULTS / f"verify4__{fig}.answer.md").write_text(answer)
                nb = _notebook(data)
                if nb is not None:
                    (RESULTS / f"verify4__{fig}.notebook.ipynb").write_text(
                        json.dumps(nb, default=str, indent=2)
                    )
                done[fig] = status
                print(f"  verify4__{fig}: {status} ({len(answer)} chars)", flush=True)
    ...
    lines = ["# Round-4 figure verification — status index\n"]
    for fig, tid in tasks:
        lines.append(f"- {fig}: `{tid}` — **{done.get(fig, 'pending')}**")
    (RESULTS / "INDEX_round4.md").write_text("\n".join(lines) + "\n")
