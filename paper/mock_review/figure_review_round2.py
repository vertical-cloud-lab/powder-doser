#!/usr/bin/env python3
"""Round-2 Edison ANALYSIS verification pass over the REVISED manuscript figures.
...
Task ids are recorded in ``figure_review_round2_task_ids.json``; fetch later
...
def main() -> None:
    ...
    ids_path = HERE / "figure_review_round2_task_ids.json"
    ...
    for fig in args.figures:
        ...
        if fig not in record["uploads"]:
            ...
            stored = client.store_file_content(
                name=f"powder_doser_{fig}_round2_bundle",
                file_path=str(bdir),
                as_collection=True,
            )
