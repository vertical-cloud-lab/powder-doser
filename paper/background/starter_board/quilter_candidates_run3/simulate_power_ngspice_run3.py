#!/usr/bin/env python3
"""ngspice power-distribution (IR-drop) simulation of the run-3 Quilter candidates.

Run-3 counterpart of ``../quilter_candidates_run2/simulate_power_ngspice_run2.py``
(note ``24``), updated for the 2026-07-15 candidates, which come from the
commit-``0683525`` board with the second dispensing-angle servo:

* the ``+5V`` rail now feeds **two** servo headers (``M3`` + ``M4``, PR #66
  dual-servo hinge / PR #100 rev C), so the worst-case +5 V load doubles to
  ~2x0.75 A near-stall + the Pico's VSYS draw -- exactly the reason the
  Quilter Power Nets row for ``+5V`` was recommended at 2000 mA;
* everything else (T-junction splitting, layer-aware node clustering,
  module-internal-bridge detection, IPC-2221 rise at the actual minimum
  routed width, +12 V solenoid-rail ``.tran``) is unchanged from run 2.
...
Usage::
    ...
    python3 simulate_power_ngspice_run3.py                 # prints the table
    python3 simulate_power_ngspice_run3.py --json out.json
    python3 simulate_power_ngspice_run3.py --decks decks/  # also dump .cir files
"""
...
LOADS = {
    ...
    "+5V": {
        "M3": 0.75,   # servo 1 header (near-stall transient)
        "M4": 0.75,   # servo 2 header (mirrored dispense servo, PR #66/#100 rev C)
...
def _emit_deck(net: str, supply_v: float, src_node: int,
    ...
    lines = [f"* ngspice power IR-drop deck for {net} (run 3)"]
...
def _transient_sag(r_feed: float) -> dict | None:
    ...
    if not shutil.which("ngspice"):
        return None
    period = 1.0 / SOL_FREQ
    deck = f"""* +12V solenoid-rail transient decoupling check (run 3)
Vsup nsrc 0 12
