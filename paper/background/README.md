...
| [`24-quilter-run2-candidate-evaluation.md`](24-quilter-run2-candidate-evaluation.md) | Generative EDA/PCB | **Build note (not Edison):** evaluation of the **2026-07-14 Quilter run-2** candidates — the post-audit 110 × 110 mm board routed with the reviewed settings (JLC04161H-7628 *with power plane*, 10/6 mil, per-net currents). Five of six committed verbatim under [`starter_board/quilter_candidates_run2/`](starter_board/quilter_candidates_run2/) (candidate 4's attachment link was malformed). The settings paid off: +12V/GND at 0.514 mm, no `(net 0)` copper, 18–25 vias, worst +12 V IR drop 178 mV, worst supply-net IPC-2221 rise 6.8 °C. Systemic finds: the In2 "power plane" zone exported with `<no net>` (source of all 154 DRC errors per board — assign it to `GND` + refill before fab), candidates 2/5 put 9–10 THT modules on the **bottom side** (disqualifying), and candidate 5 feeds the stepper's 1 A *through* SR1's carrier pour. **Candidate 6 is recommended** (all-top-side, best +12 V drop, all three bulk caps ≤ 5.8 mm from their supply pins); candidate 1 runner-up. Reproducible via [`analyze_quilter_candidates_run2.py`](starter_board/quilter_candidates_run2/analyze_quilter_candidates_run2.py) + [`simulate_power_ngspice_run2.py`](starter_board/quilter_candidates_run2/simulate_power_ngspice_run2.py) (T-junction-splitting, layer-aware mesh). |
| [`25-quilter-run3-candidate-evaluation.md`](25-quilter-run3-candidate-evaluation.md) | Generative EDA/PCB | **Build note (not Edison):** evaluation of the **2026-07-15 Quilter run-3** candidates — the first run on the **2-servo** board (commit `0683525`: `M4` on GP2 / `SERVO_SIG2`, 16 footprints / 23 nets). All six committed verbatim under [`starter_board/quilter_candidates_run3/`](starter_board/quilter_candidates_run3/). **M4 verified present + routed to GP2 on all six** (union-find over the routed `SERVO_SIG2` copper), and a **full parts audit** against PR #100 rev C's `PLACEMENTS` confirms every ref/net matches (nothing else missing). The corrected Power Nets rows landed: +12V/+5V/GND all at 0.514 mm; worst IR drop anywhere is 138 mV, solenoid rail ≥ 11.957 V. Discriminators: candidates **4/5/6 put 7–8 THT modules on the bottom side** (not buildable), and candidate **1 feeds the buck + Tic's +12 V through SR1's carrier pour** (unplug the shunt reg → stepper loses power). **Candidate 2 is recommended** (shortest buildable copper 939 mm, fewest vias 19, caps ≤ 8.2 mm, honest +12 V distribution) — with note 25's pre-fab **In2 zone fix now applied** ([`fix_candidate2_in2_zone.py`](starter_board/quilter_candidates_run3/fix_candidate2_in2_zone.py) → [`candidate_2_fixed/`](starter_board/quilter_candidates_run3/candidate_2_fixed/): `<no net>` In2 zone reassigned to `GND` + refilled; 152 DRC errors → 0, second solid GND plane, ratsnest still 0 — fabricate that board file); avoid 3 (C2 stranded 25.8 mm, worst +5 V drops). Reproducible via [`analyze_quilter_candidates_run3.py`](starter_board/quilter_candidates_run3/analyze_quilter_candidates_run3.py) + [`simulate_power_ngspice_run3.py`](starter_board/quilter_candidates_run3/simulate_power_ngspice_run3.py) (adds the M4 check, the netlist audit, and the 2-servo +5 V load). **2026-07-16 — ready to order:** [`prepare_jlcpcb_order.py`](starter_board/quilter_candidates_run3/prepare_jlcpcb_order.py) ran the final gate (headless *Update PCB from Schematic* sync pass — 175/175 netlist nodes in sync, zero rewrites; final DRC 0 errors; JLCPCB capability checks) and exported the JLCPCB fabrication kit: upload [`candidate_2_fixed/powder_doser_run3_candidate2_jlcpcb.zip`](starter_board/quilter_candidates_run3/candidate_2_fixed/powder_doser_run3_candidate2_jlcpcb.zip) at <https://cart.jlcpcb.com/quote> (order settings in note 25's *JLCPCB ordering kit* section). |
| [`26-shunt-regulator-necessity.md`](26-shunt-regulator-necessity.md) | Generative EDA/PCB | **Edison `LITERATURE_HIGH` assessment** of whether the shunt regulator **SR1** (Pololu #3776) across the +12 V rail is necessary, answering @sgbaird's [PR #76 question](https://github.com/vertical-cloud-lab/powder-doser/pull/76#issuecomment-5005495385). Worked energy-balance numbers for every rail transient source: stepper regen from the low-inertia NEMA-11 auger (~0.35 mJ, negligible), solenoid H-bridge flyback dump (worst case ~10.6 mJ → rail rises to ~18.8 V on 100 µF — still >16 V margin to the Tic's 35 V abs max), LC chopping spikes, hot-plug ringing, and the simultaneous-abrupt-stop case (mainly a **brownout**, not overvoltage, risk). Verdict: **not strictly necessary; cheap insurance worth keeping**; the leaner replacement if removed is 470–1000 µF bulk + an SMBJ15A-class TVS; the tilt-slam / servo back-drive scenario lives on the **+5 V rail, which SR1 cannot protect** (a 5.6 V TVS there is the right tool if it becomes real). Carries an editorial preamble correcting the part identity: #3776 is the **33.0 V / 4.00 Ω / 9 W** set-point version (a last-ditch Tic protector, not a rail stabilizer; the 13.2 V #3775 is the tight-clamp match for a 12 V rail). Kept verbatim. Produced by [`edison_run_shunt_regulator_necessity.py`](edison_run_shunt_regulator_necessity.py). **Resolved 2026-07-17:** the team [decided](https://github.com/vertical-cloud-lab/powder-doser/pull/76#issuecomment-5005712174) to keep SR1 populated (the $11.49 #3776 is already on hand) as near-free insurance — no board change needed, since SR1 was never removed from the netlist or any routed candidate. |
