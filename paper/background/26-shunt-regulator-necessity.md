> **Editorial note (repo, not Edison):** produced by
> [`edison_run_shunt_regulator_necessity.py`](edison_run_shunt_regulator_necessity.py)
> (Edison `LITERATURE_HIGH`, task `b92d3e4f-08a1-45c8-a6a2-5c52e55c5c94`,
> 2026-07-17) for @sgbaird's PR
> [#76 question](https://github.com/vertical-cloud-lab/powder-doser/pull/76#issuecomment-5005495385)
> on whether SR1 is necessary. **One part-identity correction to read this
> note with:** the prompt described SR1 as a "TL431-based ... ~13.2 V-class"
> clamp, but the part actually specced in PR #25 / the starter board —
> **Pololu #3776** — is the **"Shunt Regulator: 33.0 V, 4.00 Ω, 9 W"**
> (MOSFET-switched power resistor, ~0.5 mA quiescent, per its
> [product page](https://www.pololu.com/product/3776)). On a 12 V rail a
> 33.0 V set point never conducts in normal operation and only fires just
> below the Tic T500's 35 V absolute max — so the as-built SR1 is purely a
> last-ditch protector for the Tic, **not** a rail stabilizer, and the
> rows below about catching adapter overshoot near 13 V do not apply as
