...
*board* this brainstorm proposes. **The KiCad project for it is
committed alongside this document at
[`hardware/kicad/satellite-rev-a/`](../hardware/kicad/satellite-rev-a/)**
— a placed-but-unrouted Rev A capture (50 × 50 mm 2-layer FR-4,
all §3.4 components instantiated with §3.4 net names). Routing the
copper traces is intentionally a follow-up PR so the topology and
connector inventory below can be reviewed first.

This section explains the design intent that the KiCad project
implements.
...
The actual KiCad project for the satellite PCB is committed at
[`hardware/kicad/satellite-rev-a/`](../hardware/kicad/satellite-rev-a/)
— a placed-but-unrouted Rev A capture, generated by the
reproducible `generate.py` script under that directory. Net
routing on the 2-layer board is the natural first deliverable for
the follow-up PR once the §3.3 topology is approved.
