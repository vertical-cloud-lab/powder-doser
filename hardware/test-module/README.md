...
to a single **Raspberry Pi Pico (RP2040)** so the whole bench rig can be
exercised over USB-serial without bringing up a Linux stack.
...
|---|---|---|---|---|
| U2  | Raspberry Pi Pico (RP2040, 2 × 20 castellated header) | — *(new for bench rig)* | 1 | Any Pico/Pico H/Pico WH works. |
| U1  | Pololu D24V22F5 5 V / 2.5 A buck regulator | 15 | 1 | 12 V → 5 V for the Pico + servo + solenoid. |
...
| U5  | Pololu DRV8825 stepper driver carrier (#2133) | 11-alt | 1 | DRV8825 over Tic T500 for bench — the Pico can generate STEP/DIR directly. |
...
on top of whatever Pico is on the bench.
...
| Net | DRV / actuator pin | Pico pin | Notes |
|---|---|---|---|
| `+12V`       | J1.+ , U1.VIN , U4.VM , U5.VMOT , C1.+ , C3.+ | —     | 12 V from the wall brick. |
| `+5V`        | U1.VOUT , U2.VSYS , M3.+5V , C2.+              | —     | Buck output; powers Pico + servo. |
| `+3V3`       | U2.3V3 , U3.VIN , U5.VDD , U5.nSLP , U5.nRST   | —     | Pico on-board LDO; logic + DRV2605L. |
| `GND`        | (all) | —     | Common ground (DRV8825 VMOT and VDD grounds tied at the carrier). |
...
3. **MCU power.** Run `+5V → Pico VSYS (pin 39)` and `GND → Pico GND
   (any of pins 3/8/13/18/23/28/33/38)`.  Leave `VBUS` open.  Tap the
   Pico's `3V3` (pin 36) for a `+3V3` rail.

4. **I²C (DRV2605L).** Wire:
   * `Pico GP0 (pin 1) → U3 SDA`
   * `Pico GP1 (pin 2) → U3 SCL`
   * `U3 VIN → +3V3`, `U3 GND → GND`
   * `Pico GP14 (pin 19) → U3 EN` *(short to `U3 IN_TRIG` if you ever
...
5. **Solenoid (DRV8871).** Wire:
   ...
   * `Pico GP10 (pin 14) → U4 IN1`
   * `Pico GP11 (pin 15) → U4 IN2`
   * `U4 OUT1 → solenoid +`, `U4 OUT2 → solenoid -`.

6. **Stepper (DRV8825).** Wire:
   ...
   * `Pico GP2 → STEP`, `GP3 → DIR`, `GP4 → nEN`, `GP5 → M0`,
     `GP6 → M1`, `GP7 → M2`.
...
9. **First power-on.** Plug the Pico into USB *first* (so the firmware
...
## Why a Pico instead of the Pi Zero 2 W from PR #25?
...
Folding the controller into a $4 microcontroller:
...
* drops the boot-time-pull-up gotcha on the stepper `~ENABLE` line
  that #25 calls out (the Pico's GPIOs are in input mode at reset, so
  the DRV8825's on-board pull-down keeps it disabled until the firmware
  drives `STP_nEN`);
