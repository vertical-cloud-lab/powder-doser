...
SILENT_CHECKLIST = (
    ...
    "  1. TX/RX not crossed over (the #1 cause of a silent link). The\n"
    "     Pico's TX must reach the scale's RX and vice-versa:\n"
    "       GP{tx} -> U6 T1IN, GP{rx} <- U6 R1OUT, then on the RS-232\n"
    "       side U6 T1OUT -> J2 RXD and U6 R1IN <- J2 TXD.\n"
    "     Swapping a single pair gives exactly this silent result.\n"
    "  2. Wrong UART / pins. This build uses UART{uart} on GP{tx}/GP{rx};\n"
    "     make sure the wires land there (not the Tic's UART1 GP4/GP5).\n"
    "  3. MAX3232 unpowered. U6 VCC must be on +3V3 and U6 GND on GND.\n"
