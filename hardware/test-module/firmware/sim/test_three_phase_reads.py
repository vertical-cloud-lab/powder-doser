...
def test_refused_tare_with_full_cup_refuses_to_dose():
    """A refused tare must not report leftover cup powder as delivered."""
    clock = VirtualClock()
    column = Column(g_per_rev=0.230, start_pan_g=1.541)   # the 08-03 cup
    ...
    check("refused tare + full cup is NOT_TARED",
          res.status == m3p.DoseResult.NOT_TARED,
          "got {}".format(res.status))
    check("refused tare + full cup reports no delivery",
          res.dispensed_g == 0.0,
          "reported {:.4f} g".format(res.dispensed_g))
    check("refused tare + full cup never turns the auger",
...
def test_bracket_read_shape_is_sane():
    ...
    check("a bracket read costs about a second, not a minute",
          ...
          <= 3.0)
    check("baseline limit is far above drift, far below a loaded cup",
          0.010 <= m3p.DOSE_MAX_BASELINE_G <= 0.100)


def main():
    for fn in (test_balance_never_stable_still_doses,
               test_old_path_fails_where_new_one_works,
               test_refused_tare_with_full_cup_refuses_to_dose,
