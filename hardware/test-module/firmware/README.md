# Test-module firmware (CircuitPython, Raspberry Pi Pico)
...
| `config.py` | Pin map + every adjustable parameter.  Edit & save -> Pico reloads. |

## Install

1. **Flash CircuitPython 9.x** for the Raspberry Pi Pico (UF2 from
...
2. The Pico re-enumerates as `CIRCUITPY`.  Copy this folder's
...
| Stepper  | `STEPPER_SPEED_RPM`      | Step rate (5..240 is safe with item 18 shunt regulator). |
