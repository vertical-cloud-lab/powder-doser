...
The run document also records *where to watch the run*: the bench camera
streams continuously, so ``video`` carries the covering broadcast and the
``?t=`` offsets of the run, each block and each dose (issue #148).  That
mapping used to be reconstructed by hand afterwards; resolving it at
capture time keeps it with the data, and keeps it from depending on the
broadcast still being findable months later.  It is best effort --
``scripts/stream_anchor.py`` never lets a slow or unreachable YouTube
break a capture, and ``--no-stream-link`` skips it entirely.
...
try:
    import stream_anchor            # scripts/stream_anchor.py, beside this
except ImportError:                 # analysis-only checkouts may not have it
    stream_anchor = None
...
# every UTC stamp; 4 adds the video block (issue #148).  Every earlier
# field is unchanged and still populated, so schema_version 1, 2 and 3
# documents stay readable by the same consumers.
SCHEMA_VERSION = 4
...
# ---------------------------------------------------------------------------
# Video (issue #148: which livestream, and where in it)
# ---------------------------------------------------------------------------

def attach_video(doc, args):
    """Record where in the bench livestream this run can be watched.

    Deliberately the *last* thing that happens to a run document before it
    is written: it is the only step that touches the network, and a run
    that has already finished must not be put at risk by YouTube being
    slow.  Every failure mode -- no yt-dlp, no network, no broadcast
    covering the run, a listing that has not caught up with a stream that
    started minutes ago -- lands in ``video.error`` and leaves the rest of
    the document untouched, so the run is still written, still uploaded,
    and can be backfilled afterwards with::

        python scripts/stream_anchor.py --run-json run_<id>.json --push
    """
    if getattr(args, "no_stream_link", False):
        return None
    if not doc.get("started_utc"):
        # --from-raw without --started-utc: no wall clock, no anchor.
        return None
    if stream_anchor is None:
        doc["video"] = {"error": "stream_anchor.py not importable"}
        return doc["video"]
    block = stream_anchor.resolve_for_document(
        doc, camera=getattr(args, "stream_camera", "picam-d1pr"),
        timeout_s=getattr(args, "stream_timeout", 90))
    doc["video"] = block
    if "error" in block:
        print("[video] no link resolved ({}) -- backfill later with "
              "scripts/stream_anchor.py --run-json".format(block["error"]))
    else:
        print("[video] {}".format(block["url"]))
    return block
...
def main(argv=None):
    ...
    parser.add_argument("--no-stream-link", action="store_true",
                        help="skip resolving the bench livestream link "
                        "(the only network step outside --upload)")
    parser.add_argument("--stream-camera", default="picam-d1pr",
                        help="camera whose broadcast covers this rig")
    parser.add_argument("--stream-timeout", type=int, default=90,
                        help="seconds to allow for stream link lookup")
