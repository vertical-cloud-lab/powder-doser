"""Adversarial Edison spot-check of the sheet-vs-corner-pads reasoning.

Submits two tasks against the Edison Scientific platform
(``https://api.platform.edisonscientific.com``) asking it to *refute* the
shape-factor argument in ``docs/balance-isolation-slab-sourcing.md``:

* ``LITERATURE_HIGH`` (paperqa3-high) -- ground the rubber-engineering
  claims (Gent-Lindley shape factor, bonded vs. unbonded platens,
  dynamic-vs-static stiffness of high-loss viscoelastics) in primary
  sources.
* ``ANALYSIS`` (data-analysis-crow-high) -- independently recompute the
  numbers and stress-test the assumptions with code.

Usage (from the repo root)::

    python scripts/edison_isolator_spotcheck.py submit
    python scripts/edison_isolator_spotcheck.py wait     # blocking poll
    python scripts/edison_isolator_spotcheck.py fetch    # dump artifacts

The API key is read from ``EDISON_PLATFORM_API_KEY`` and passed to
``EdisonClient`` explicitly; it is never printed.
"""
...
OUT = Path(__file__).resolve().parent.parent / "outputs" / "isolator-spotcheck"
...
def client() -> EdisonClient:
    key = os.environ.get("EDISON_PLATFORM_API_KEY")
    if not key:
        sys.exit("EDISON_PLATFORM_API_KEY is not set")
...
def submit() -> None:
    ...
    for label, job, query in (
        ("literature", JobNames.LITERATURE_HIGH, LIT_QUERY),
        ("analysis", JobNames.ANALYSIS, ANALYSIS_QUERY),
    ):
        tid = c.create_task({"name": job, "query": query})
        tid = tid[0] if isinstance(tid, list) else tid
        ids[label] = str(tid)
        print(f"submitted {label} ({job}): {tid}", flush=True)
    (OUT / "_task_id.json").write_text(json.dumps(ids, indent=2))
    (OUT / "query-literature.md").write_text(LIT_QUERY)
    (OUT / "query-analysis.md").write_text(ANALYSIS_QUERY)


def wait(poll_s: int = 120, deadline_s: int = 2100) -> None:
    c = client()
    ids = json.loads((OUT / "_task_id.json").read_text())
    start = time.time()
    done: dict[str, str] = {}
    while True:
        for label, tid in ids.items():
            if label in done:
                continue
            try:
                status = str(c.get_task(task_id=tid).status).split(".")[-1].lower()
            except Exception as exc:  # noqa: BLE001
                status = f"(poll error: {type(exc).__name__})"
            print(f"[{int(time.time()-start):5d}s] {label}: {status}", flush=True)
            if status in TERMINAL:
                done[label] = status
        if len(done) == len(ids) or time.time() - start > deadline_s:
            break
        time.sleep(poll_s)
    print("final:", json.dumps(done), flush=True)
...
def fetch() -> None:
    ...
    for label, tid in ids.items():
        try:
            task = c.get_task(task_id=tid, verbose=True)
        except Exception as exc:  # noqa: BLE001
        ...
        body = f"<!-- task_id: {tid}  status: {status} -->\n\n{answer}\n"
        (OUT / f"{label}-answer.md").write_text(body)
        print(f"{label}: status={status} answer_chars={len(answer)}", flush=True)
        try:
            dump = task.model_dump(mode="json")
            (OUT / f"{label}-task.json").write_text(json.dumps(dump, indent=2, default=str))
        except Exception as exc:  # noqa: BLE001
            print(f"{label}: verbose dump failed ({type(exc).__name__})", flush=True)
