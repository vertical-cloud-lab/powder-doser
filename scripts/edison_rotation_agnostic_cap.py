"""Submit Edison Scientific queries on rotation-agnostic front caps for the
multi-doser carousel (issue #128).

Two tasks are submitted in parallel:

* ``PRECEDENT`` (``job-futurehouse-paperqa3-precedent``) — patent/prior-art
  oriented, to surface *real, existing* mechanisms (iris diaphragms, capless
  fuel fillers, self-closing septa, ratcheting closures) that solve the
  "actuate a closure without knowing the rotational orientation of the part"
  problem.
* ``LITERATURE_HIGH`` (``job-futurehouse-paperqa3-high``) — literature-deep,
  focused on powder-handling / contamination / sealing performance of such
  closures for cohesive metal powders.

Artifacts (task JSON, answer, references) are written next to this script's
output directory, ``docs/edison/`` per repo convention.
...
OUT_DIR = ROOT / "docs" / "edison" / "multidoser-cap"
TASK_FILE = OUT_DIR / "_task_ids.json"
...
TASKS = {
    "precedent": (JobNames.PRECEDENT, PRECEDENT_PROMPT),
    "literature": (JobNames.LITERATURE_HIGH, LITERATURE_PROMPT),
}
...
def _client() -> EdisonClient:
    ...
    if not key:
        print("ERROR: EDISON_PLATFORM_API_KEY not set", file=sys.stderr)
        raise SystemExit(2)
...
def submit() -> None:
    ...
    ids = {}
    for key, (job, prompt) in TASKS.items():
        tid = client.create_task({"name": job, "query": prompt})
        print(f"{key}: {tid}", flush=True)
        ids[key] = str(tid)
    TASK_FILE.write_text(json.dumps(ids, indent=2) + "\n")
    (OUT_DIR / "_prompts.md").write_text(
        "\n\n---\n\n".join(
...
def _dump(client: EdisonClient, key: str, tid: str) -> str:
    ...
    answer = getattr(task, "formatted_answer", None) or getattr(task, "answer", "") or ""
    (OUT_DIR / f"{key}.answer.md").write_text(str(answer) + "\n")
    return status


def wait(poll_s: int = 120, max_s: int = 2700) -> None:
    client = _client()
    ids = json.loads(TASK_FILE.read_text())
    ...
    while time.time() < deadline:
        for key, tid in ids.items():
            if key in done:
                continue
            status = _dump(client, key, tid)
            print(f"{key}: {status}", flush=True)
            if status.lower().split(".")[-1] in TERMINAL:
                done[key] = status
        if len(done) == len(ids):
            break
        time.sleep(poll_s)
    print("final:", json.dumps(done, indent=2))
