...
def speeds(run: dict, blocks: str | None = None) -> str:
    """Auger speeds, restricted to the blocks that actually ran.

    Both RPM parameters are configured on every run whether or not their
    block runs, so a dose-only run (Block G/H) was reporting a C/E/D speed
    ...
    parts = []
    if rot and (blocks is None or set("CE") & set(blocks)):
        parts.append(f"C/E {float(rot):g}")
    if sweep and (blocks is None or "D" in blocks):
...
def row_for(run: dict, broadcasts, anchors) -> dict:
    ...
    return {
        ...
        "speeds_rpm": speeds(run, blocks_run(run)),
