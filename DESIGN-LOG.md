...
**Rationale:** Establish a fully mechanical, gantry-only dosing concept that needs no second actuator while fighting micron-powder adhesion (PTFE + ERM) and closing the dose loop gravimetrically.
...
**Rationale:** Turn the hand sketch into a shared visual baseline so reviewers could critique a concrete concept before details were fixed.
...
**Rationale:** Fold in review corrections (missing pin, two arms, vertical arm) and reframe the concept for cohesive micron powders and printable parts.
...
**Rationale:** Match the reviewer's ferris-wheel intent so the trough rotates end-over-end on a transverse pin instead of the wrong edges.
...
**Rationale:** Improve fill consistency and dose repeatability with a longitudinal sideways tilt and J-curve scoop-drag, without adding a bucket actuator.
...
**Rationale:** Make the kinematic constraint explicit by letting a pin-in-slot path define rotation under the no-second-axis gantry constraint.
...
**Rationale:** Purely a legibility pass: line up the mechanism panels/GIF and clarify connection points without changing the mechanism.
...
**Rationale:** Explore a passive bistable (snap-through flexure) route to two stable scoop/dump poses as an alternative to cam actuation.
...
**Rationale:** Make the compliant mechanism physically testable by encoding the flexure and pre-compression as a printable part.
...
**Rationale:** Simplify cam/strike-off contact and remove special-case powder traps by using a continuous chamfered rim instead of a spout.
...
**Rationale:** Move from sketches to a parametric CAD/DFM pipeline so the concept could be measured, checked, and regenerated.
...
**Rationale:** Fix a modeling defect (floating apex carrier) so the printed flexure is physically connected before final renders.
...
**Rationale:** Reconcile statics, cam-ramp geometry, and documentation mismatches flagged in review to leave a consistent final PR #2 CAD state.
...
**Rationale:** Prototype the Edison-preferred ERM-sieve direction as buildable parts within a one-day workshop scope.
...
**Rationale:** Establish the vertical Archimedes screw-feed architecture as the primary direction before printability work.
...
**Rationale:** Make the auger actually manufacturable — manifold, supported internal helix, correctly print-oriented.
...
**Rationale:** Provide proof visuals that the plain-looking cylinder really contains the helix/funnel/slots the slicer could not show.
...
**Rationale:** Adopt a more robust, conventional screw-in-tube split (fixed shaft, rotating housing) matching the intended real design.
...
**Rationale:** Shrink the part so it prints within the remaining workshop time while preserving the fits and helix.
...
**Rationale:** Keep actuation purely gantry-mechanical by tapping an anvil to meter powder through sieve holes (Alternative A).
...
**Rationale:** Explore discrete volumetric dosing by indexing Pez-like chambers past a dispense point (Alternative B).
...
**Rationale:** Test whether contact pickup plus wipe-off can meter powder without a screw (Alternative C).
...
**Rationale:** Test whether bristles/teeth can carry and release cohesive powder mechanically (Alternative D).
...
**Rationale:** Trade dose precision for mechanical simplicity using shaker-style agitation through holes (Alternative E).
...
**Rationale:** Carry the auger screw-feed principle into a compact, comparable alternative for side-by-side evaluation (Alternative F).
...
**Rationale:** Add continuous bounded ERM vibration to better match published vibratory-sieve regimes (Alternative G, ranked highest by Edison).
...
**Rationale:** Isolate the higher-risk solenoid-tap actuator as a distinct option to compare against passive and ERM sieving (Alternative H).
...
**Rationale:** Make each alternative's function legible (numbered parts + 3-step cycle) so the set could be understood and chosen.
...
**Rationale:** Replace ambiguous block/ASCII diagrams with a real KiCad schematic plus explicit connection assumptions for review.
...
**Rationale:** Improve schematic readability by pulling net labels off the symbol bodies while keeping the functional grouping.
...
**Rationale:** Check whether a shared inward-pointing cup for 12 channels is geometrically plausible at realistic dimensions before committing.
...
**Rationale:** Maximize capacity and dodge hopper bridging/ratholing by reverting to a larger monolithic integrated auger sized for an H2D printer.
...
**Rationale:** Ensure powder can actually be loaded by making the top slots break through to the outside surface.
...
**Outcome** ➡️ Superseded — the four small loading slots were later replaced by a single large top opening with a pour lip ([v6](#e061)).
...
**Rationale:** Anchor the A–H mechanisms in real CAD so their spatial relationships and load/dispense positions stay consistent.
...
**Rationale:** Establish a repeatable single-channel module archetype that can scale to many powders (Idea B).
...
**Rationale:** Seal a stored channel bore mechanically with a printable twist-lock plug to prevent spill and cross-contamination.
...
**Rationale:** Explore a normally-closed hatch the downstream mechanism can open, as an alternative sealing primitive.
...
**Rationale:** Prove a rotating twin-disc shutter can toggle the channel outlet between sealed and open.
...
**Rationale:** Resolve printability/usability review (motor blocking entry, unintuitive frame) with a spine, belt drive, and swappable cartridge.
...
**Rationale:** Make powder flow and tilt legible, and flag that swaps must replace rotor plus cartridge to avoid contamination.
...
**Rationale:** Use Edison VLM-Judge rounds to fix the mechanically nonsensical, floating, and uncoupled parts flagged in review.

<img src="https://raw.githubusercontent.com/vertical-cloud-lab/powder-doser/512c0af608c34985f16d03d6c5e45577f5c740b2/design/cad/single-channel-module/renders/single_channel_module_iso.png" width="300"> <img src="https://raw.githubusercontent.com/vertical-cloud-lab/powder-doser/512c0af608c34985f16d03d6c5e45577f5c740b2/design/cad/single-channel-module/renders/single_channel_module_powder_flow.png" width="300">

**Outcome** ⚠️ Edison round 2 caught new geometry bugs introduced by these edits (bracket/rotor interference, belt-plane height, clearances), fixed in [v3-r2](#e042).
...
**Rationale:** Clear the new geometry bugs the judge caught (interference, belt-plane, clearances) that the prior pass introduced.
...
**Rationale:** Lower the pivot to the outlet and strengthen/scale the bracket and belt per review, with real assembly STLs and scale context.
...
**Rationale:** Give the unsupported inner screw material to land on (a central shaft) to fix the strings/print failure.
...
**Outcome** ⚠️ Fixed the unsupported-screw print failure, but the added central core risked trapping powder; superseded by the hollow-core [v5](#e045).
...
**Rationale:** Remove the central core to stop powder trapping and reduce cantilever, restoring a flow-friendly hollow bore.
...
**Rationale:** Open the blocked bayonet housing holes and verify engagement via assembly, interference, and exploded views.
...
**Rationale:** Make the hatch buildable and understandable as one PETG living-hinge body with explanatory views.
...
**Rationale:** Give the twin shutter discs a defined pivot/retainer (M3 screw + captive nut) so they cannot fall apart.
...
**Rationale:** Produce the first printable shaft-collar bracket to the sketch; bore sized provisionally pending a real auger reference.
...
**Rationale:** Tie the clamp ears into the collar wall so the tightening tabs cannot shear off the top.
...
**Rationale:** Center the M3 clamp hole with wall above and below and shorten the screw path through thinner tabs.
...
**Rationale:** Re-size the bracket to the real 25 mm PR #16 auger (25.5 mm running bore) instead of the assumed small shaft.
...
**Rationale:** Add an external gear band one-third toward the exit plus a NEMA pinion to drive the auger; first geared CAD for review.
...
**Outcome** ❌ Print and review found the auger internals had been altered and the gear/pinion layout was too small for motor clearance; superseded by [v2](#e055).
...
**Rationale:** Lay out a reviewable topology-C satellite PCB (RP2040 + drivers) before committing to copper routing.
...
**Rationale:** Restore powder flow and motor clearance with an open gear bore, a larger 48T gear, and a 32 mm center distance.
...
**Outcome** ⚠️ The gear meshed and the motor cleared, but slicer inspection found the internal auger core missing; superseded by full-length [v3](#e056).
...
**Rationale:** Restore the missing internal helix/shaft and share a common core so the full and short variants cannot diverge.
...
**Rationale:** Provide a shorter alternate (250→180 mm) while keeping the gear-to-exit distance and a verified continuous helix.
...
**Rationale:** Thicken the bracket plate (4→14 mm) to lift the Ø50 mm gear clear of the baseplate.
...
**Rationale:** Build an independent tap collar (coin motor + solenoid) with a cord-winding hardstop per the issue sketch.
...
**Rationale:** Lay a CADsmith foundation tying all parts together with a hinge on the discharge axis and 0–90° tilt.
...
**Rationale:** Make loading less fiddly with one large opening plus a pour lip, kept minimal and non-invasive.
...
**Outcome ✅ — printed & bench-tested** (issue [#16](https://github.com/vertical-cloud-lab/powder-doser/issues/16), @sgbaird): the auger printed cleanly (oriented upside-down to avoid a floating internal region) and was run with NaCl. ▶ [NaCl dispensing test](https://youtu.be/5nx-SDRfRgg)
...
**Rationale:** Match the sketch with a one-piece printable shaft-collar bracket toleranced to the 25 mm auger.
...
**Rationale:** Provide a CADsmith-style CadQuery split-collar-plus-flange bracket around the real PR #16 auger.
...
**Rationale:** Return to a single-piece, drawing-faithful profile by removing the pyramidal base via one uniform extrusion.
...
**Rationale:** Establish a compact printable foundation with through-holes, a discharge-axis hinge, and cup/scale room (placeholders).
...
**Rationale:** Establish the dispense-axis hinge concept on a baseplate from the module layout, with rotation and flow visuals.
...
**Rationale:** Replace nonsensical placeholders with real PR #49/#51/#55 geometry so the motor pinion actually meshes the auger gear.
...
**Rationale:** Mount the real imported parts correctly so gear mesh, bracket holes, and tap-collar placement are physically valid.
...
**Rationale:** Remove collar/plate interference, put the actuator holes over solid material, and strengthen the fragile motor pads.
...
**Rationale:** Keep the hardstop flat (countersink), free collar rotation (plate relief), and reinforce pads using the bracket template.
...
**Rationale:** Stand up a one-microcontroller bench rig to exercise angle, tap, vibration, and rotation from the #25 parts.
...
**Rationale:** Produce the printable plate/baseplate/hinge with 0–90° motion and document hole spacing inferred from the real parts.
...
**Rationale:** Match the mount-plate depth to the tap collar (12→18 mm) and lock them parametrically so they align on the auger axis.
...
**Rationale:** Realign to the issue #62 drawing: clear side, correct order, motor axis parallel, and hinge through the dispense point.
...
**Rationale:** Realign the layout to the drawing with a clear side and the hinge through the auger dispensing point.
...
**Rationale:** Adopt a front-face NEMA mount, raise the auger on plinths to drop the through-plate slot, and open a front U-notch.
...
**Rationale:** Front-face-mount the motor, raise the auger to remove the gear slot, split into two side hinges, and leave the discharge open.
...
**Rationale:** Center the auger and gap, equalize and thicken the hinges, and bolt the brackets directly to the plate.
...
**Rationale:** Symmetrize the plate, grow equal hinge lobes, drop the plinths, and clear the underside of actuator features.
...
**Rationale:** Re-attempt CADsmith now that the API key exists, while keeping the hand-authored assembly as the source of truth.
...
**Rationale:** Pack the latest lifted brackets/tap-collar/gear to ~1 mm, align the pinion, and rework the hinges into a sandwich.
...
**Rationale:** Align the bores with updated lifted brackets, pack the front stack to ~1 mm, and build robust sandwich hinges on an M5 pin.
...
**Rationale:** Delete the obsolete linear-actuator clevis and give the hinge stands full baseplate contact instead of a cantilever.
...
**Rationale:** Remove the unused actuator stand and extend the hinge arms for continuous baseplate contact.
...
**Rationale:** Switch from a linear actuator to a 2:1 servo-driven hinge gear, checking baseplate/plate interference.
...
**Rationale:** Use proper involute teeth (per #49), slope the hinge backs for sweep clearance, and anchor the servo wall against loads.
...
**Rationale:** Open the closed exit by tapering the funnel shaft (Ø8→Ø1 mm) so an annular powder path clears around it.
...
**Outcome** ✅ Printable with a cleared exit path; review then asked to also carry the screw through the funnel ([v2](#e089)).
...
**Rationale:** Correct the servo holes and head to the precise MG996R diagram while keeping the 2:1 mesh at the same center distance.
...
**Rationale:** Keep a flight surface through the funnel by continuing the helix down the tapered annulus to the exit.
...
**Outcome** ⚠️ Printable, but the funnel and upper helix phases were mismatched at the seam; fixed in [v3](#e090).
...
**Rationale:** Fix the funnel/upper twist-hand mismatch so the spiral is continuous, verified by a top-down cross-section.
...
**Rationale:** Raise the servo spline to the true 10 mm height and back-solve the gear module to keep a 2:1 mesh without interference.
...
**Outcome ✅ — hardware test** (issue [#65](https://github.com/vertical-cloud-lab/powder-doser/issues/65), @swcharles): the MG996R servo drove the printed mounting-plate hinge through its angle change on the baseplate. ▶ [Servo angle-change video](https://youtu.be/ASQb1NmKOFs)
...
**Rationale:** Standardize on a Pico W, realign labels for readability, add the #25 shunt regulator, and ramp the servo moves smoothly.
...
**Rationale:** Match the real 2×8 DRV8825 carrier (no VDD) and drop the redundant Tic T500 since the Pico W handles STEP/DIR.
...
**Outcome ✅ — bench tests** (issue [#60](https://github.com/vertical-cloud-lab/powder-doser/issues/60), @williamulbz): the assembled single-Pico-W module drove each actuator following the firmware/wiring docs from PR #61. ▶ [Haptic motor](https://youtu.be/hIK3kkgJRD4) · ▶ [Servo control](https://youtu.be/NQX6OudTasM) · ▶ [Solenoid control](https://youtu.be/HvvvGUM0XXY)
...
**Rationale:** Bench-test nozzle type 1 gearless (production inlet/outlet, ≥3 turns) to characterize a direct-cutoff funnel exit.
...
**Rationale:** Bench-test nozzle type 2 to isolate small-gap pre-nozzle outlet behavior, gearless and short.
...
**Outcome ❌ — NaCl test** (issue [#48](https://github.com/vertical-cloud-lab/powder-doser/issues/48), @swcharles): type 2 did not work for salt — the small pre-nozzle gap let it rattle like a maraca with the crystals stuck at the end; smaller powders might still pass. ▶ [Auger 2](https://www.youtube.com/watch?v=T0gh_4o2ak8)
...
**Rationale:** Bench-test nozzle type 3's tapered-tip metering without continuing the helix to the outlet.
...
**Outcome ✅ — NaCl test** (issue [#48](https://github.com/vertical-cloud-lab/powder-doser/issues/48), @swcharles): type 3 worked well, though crystals might jam in the small gap when fully vertical. ▶ [Auger 3](https://www.youtube.com/watch?v=lMtbQKnj60g)
...
**Rationale:** Bench-test nozzle type 4, combining the type 2 and type 3 ideas (continued screw plus tapering shaft) for combined metering.
