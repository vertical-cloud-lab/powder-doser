...
### After servo/tilt maintenance: confirm the plate physically moves

The firmware confirms the servo *signal* was sent, never that the plate
*moved* — there is no plate encoder. The 2026-08-11 AlSi10Mg run was lost
to exactly this: the servos had seized, the plate stayed at 0° for the
whole battery, and every `tilt_deg` label was commanded-only
([amendment](battery-runs/2026-08-11-alsi10mg.md)). A 0°-connectivity
servo sweep does *not* catch a slipping horn, so do not treat it as proof.

After any servo/tilt work, verify the plate physically tilts **before**
committing a run, two independent ways:

1. **Camera** — command the plate to vertical and grab a frame:

   ```bash
   # on the Pi: hold vertical, e.g. mpremote ... exec "import main_three_phase as m3p; m3p.Servo().move_to(45.0)"
   python scripts/bench_frame.py --video-id <live-id> --seconds-ago 90 --out /tmp/tilt.png
   ```

   A just-started broadcast has no fetchable segment at the live edge, so
   grab from ~90 s behind. The overlay clock confirms the frame is current.

2. **In-run** — block C feed factor must climb with tilt. Three near-equal
   values across 0/45/90° means the plate is not moving; a monotonic curve
   means it is. This is the definitive check, at zero extra cost.

The 2026-08-12 salt repeat ([notes](battery-runs/2026-08-12-salt-repeat.md))
used both to confirm the servo repair before running.
