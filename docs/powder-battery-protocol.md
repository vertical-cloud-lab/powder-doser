...
auto-answer `keep`), so the run never blocks on a keyboard. The person
who loaded the powder should **empty the collection cup and confirm the
auger is loaded before commenting** — that's the one manual step the
unattended run can't do.
